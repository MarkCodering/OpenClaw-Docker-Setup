...
PULL_IMAGE_IF_MISSING="${PULL_IMAGE_IF_MISSING:-true}"
...
is_true() {
  [[ "$(lower "${1:-}")" =~ ^(1|true|yes|y|on)$ ]]
}

is_false() {
  [[ "$(lower "${1:-}")" =~ ^(0|false|no|n|off|skip)$ ]]
}
...
validate_inputs() {
  ...
  [[ "${WAIT_TIMEOUT_SECONDS}" =~ ^[0-9]+$ ]] || err "WAIT_TIMEOUT_SECONDS must be an integer"
}
...
ensure_image_available() {
  ...
  if is_false "${PULL_IMAGE_IF_MISSING}"; then
...
setup_codex_auth() {
  ...
  case "$(lower "${setup_choice}")" in
    ...
    1|yes|true|y|on)
      setup_choice="yes"
      ;;
    2|no|false|n|off|skip)
      return 0
      ;;
    *)
      err "Invalid SETUP_CODEX_AUTH value: ${SETUP_CODEX_AUTH}"
      ;;
  esac
...
wait_for_container_running() {
  wait_until \
    ...
    2 || err "Container did not reach a running state"
}
...
extract_token() {
  docker_exec "grep -o '\"token\": \"[^\"]*\"' '${CONFIG_PATH_IN_CONTAINER}' 2>/dev/null | head -n1 | sed 's/\"token\": \"//; s/\"$//'" \
    || true
}
...
wait_for_and_approve_device() {
  ...
  if [[ ! "${approve}" =~ ^(1|true|yes|y|on|ask)$ ]]; then
    return 0
  fi
